...
dmginfo:SetDamage(GetConVar( "sk_plr_dmg_flare_round" ):GetInt())
...
dmginfo:SetInflictor( self )
	...
	for k, v in pairs( ents.FindInSphere( self.Flare:GetPos(), 12 ) ) do
		if IsValid( v ) and ( v:IsNPC() or v:IsPlayer() ) and v != self.Owner and v:Health() > 0 and self.Flare:Health() > 40 then
			v:Ignite(30, 1 )
			if self.Flare:IsOnGround() then
			else
			v:TakeDamageInfo( dmginfo )
			self.Flare:SetHealth(30);
			self.Flare:SetVelocity(-(self.Flare:GetVelocity() * 0.9))
			 end
