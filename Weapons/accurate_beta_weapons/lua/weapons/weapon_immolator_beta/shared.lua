...
function SWEP:Think()
	if SERVER then
	if self.Weapon:GetNextPrimaryFire() < 0.1 + CurTime() then
...
local explodebeam = (self.Owner:GetEyeTrace().HitPos + Vector(beamx,beamy,beamz) * burnradius)
	...
	beamend = ents.Create("env_beam")
		beamend:SetKeyValue("renderamt", "255")
		beamend:SetKeyValue("rendercolor", "0 64 128")
		...
		beamend:SetKeyValue("spawnflags", 769 )
	...
	beam:Fire("kill", "", 0.01)
	entItem:Fire("kill", "", 0.01)
	entItem2:Fire("kill", "", 0.01)
	entItem3:Fire("kill", "", 0.01)
